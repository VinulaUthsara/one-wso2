...
/** Assigned auditor's decision on a population that passed internal review (see population/validate). */
export function usePopulationValidate() {
  ...
  return useMutation({
    mutationFn: async ({ auditId, controlId, decision, comment }: DecisionPayload): Promise<{ status: string }> => {
      const res = await authFetch(
        `${BACKEND_BASE_URL}/api/v1/audits/${auditId}/controls/${controlId}/population/validate`,
        {
          method: "POST",
