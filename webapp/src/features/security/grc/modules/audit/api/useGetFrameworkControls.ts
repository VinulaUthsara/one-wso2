...
import type { AuditFrameworkControl, FrameworkControlListResponse } from "@features/security/grc/modules/audit/types/audit";
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AuditFrameworkControl[]> => {
      const res = await authFetch(
        `${BACKEND_BASE_URL}/api/v1/audits/frameworks/controls?frameworkId=${frameworkId}`,
      );
