...
/** Internal reviewer's decision on a submitted population (see population/review). */
export function usePopulationReview() {
  ...
  return useMutation({
    mutationFn: async ({ auditId, controlId, decision, comment }: DecisionPayload): Promise<{ status: string }> => {
      const res = await authFetch(
        `${BACKEND_BASE_URL}/api/v1/audits/${auditId}/controls/${controlId}/population/review`,
        {
          method: "POST",
